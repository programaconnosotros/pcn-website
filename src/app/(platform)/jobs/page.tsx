...
const jobsData: Job[] = [
  {
    ...
    salary: "USD Salary",
    posted: "hace 1 mes",
    description: "Lead and mentor an engineering team to build AI-powered content automation solutions for top US clients. Strong skills in APIs, Python, JavaScript & LLM workflows required.",
      ...
      <div className="flex flex-col space-y-6">
        ...
        {filteredJobs.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground mt-2">{TEXTS.noJobsDescription}</p>
            <Button
              variant="outline"
              className="mt-4"
              onClick={() => {
...
function JobCard({ job }: { job: Job }) {
  return (
    <Card
      className={`overflow-hidden transition-all hover:shadow-md flex flex-col h-full relative ${!job.available ? "opacity-75" : ""}`}
    >
      {!job.available && (
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-md overflow-hidden flex-shrink-0">
            <img
              ...
              className={`w-full h-full object-cover ${!job.available ? "grayscale" : ""}`}
            />
          </div>
          <div>
      ...
      <CardContent className="pb-2 flex-grow">
        <div className="space-y-3">
          <div className="text-sm line-clamp-2 text-muted-foreground">{job.description}</div>
      ...
      <CardFooter className="pt-2 mt-auto">
        {job.available ? (
          <Button className="w-full transition-all duration-300 hover:bg-primary/90 hover:scale-[1.02] hover:shadow-md">
            {TEXTS.applyNow}
          </Button>
        ) : (
