...
6. Una vez levantado los dos contenedores (el de base de datos y el web), debes ejecutar las migraciones:
    - Si posees MacOs o Linux simplemente ejecuta el siguiente comando:
    ```bash
    make apply-migrations
    ```
    - Si posees windows creo que no puedes utilizar el comando `make` asi que lo mejor es que ejecutes lo siguiente:
    ```bash
